...
Runtime descriptors accept the optional additive `coreBuildId` field. For supported official SpatialEMU Windows, macOS, iOS, iPadOS, visionOS, tvOS, and GameEMU Android FBNeo requests, `coreBuildId` and legacy `coreSha256` are diagnostic fields and do not gate resolution. Approval still requires the exact client/platform/architecture, `libretro/fbneo` route, immutable ROM profile, and complete dependency closure. Other cores keep their existing identity rules. MAME remains strict through its audited runtime version and `contentSet`.
...
MAME and FBNeo profiles remain target-specific and audited. Adding one client identity does not make another client's arcade profile portable: each target needs a profile matching its exact client identity and the `libretro/fbneo` runtime route. Current Apple builds report `mame/0.287/mame-0.287`. FBNeo profiles may retain `coreBuildId` or legacy `coreSha256` values for diagnostics, while MAME content-set and Android Flycast policies remain independent and strict.
...
Legacy FBNeo builds may report an application-derived `coreSha256`, and newer builds may report `coreBuildId`. The server records and echoes either value for diagnostics, but neither is required for FBNeo resolution and neither can bypass the remaining client, route, ROM, or dependency checks.

Point Blank clients use the identity format `fbneo:archive-{archiveSHA256}:{targetABI}:{buildFlavor}`, encoded as lowercase ASCII. `archiveSHA256` identifies the packaged FBNeo archive used by the target, not the SpatialEMU application executable. Current clients report these values for diagnostics:
...
The archive digest, target ABI, and build flavor remain useful compatibility telemetry, but a changed or unknown value does not block an otherwise approved FBNeo launch. Clients send `coreBuildId` only when server capabilities advertise both `gameLaunchResolver: true` and `stableRuntimeIdentityV1: true`; otherwise they preserve the legacy `coreSha256` tuple.
