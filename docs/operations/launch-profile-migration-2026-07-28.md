...
Post-write verification must include a successful mobile resolver request using exactly `mame/0.287/mame-0.287` and a successful FBNeo request for each supported client/platform. Current release behavior supersedes the original fingerprint gate: changing only an official client's FBNeo SHA-256 or `coreBuildId` must still resolve successfully. Wrong runtime routes or missing dependencies must remain diagnostic failures. Release 1.01 also supports [reviewed MAME content-equivalent versions](mame-content-equivalence.md).
