...
- `foliospace.resolve_game_launch_profile`: post an exact client/runtime inventory for a `gameId` and return a stable profile with `launchProfileId`, `profileRevision`, the selected runtime tuple, logical file names, a required SHA-1 for every file, and the complete canonical dependency closure. Canonical Apple device identities are `SpatialEMU.iOS`/`ios-arm64`, `SpatialEMU.iPadOS`/`ipados-arm64`, `SpatialEMU.visionOS`/`visionos-arm64`, and `SpatialEMU.tvOS`/`tvos-arm64`, all with `architecture: arm64`; simulator and generic Apple placeholders are not accepted. Physical Apple clients must provide the exact 64-character SHA-256 for each reported Libretro core; PSP normalizes to `ppsspp` and DOS to `dosboxpure`. A PC client may report MAME and FBNeo together in `runtimes`; the server selects a matching audited profile by stable server-side priority, independent of request order, and returns the selected tuple in `runtime`. Ordinary console games reuse their validated manifests and known Libretro platform/core pairing; FBNeo and MAME remain target-specific, strict, and audited. Curated DOS responses include `dosLaunch`, while ambiguous entries return `launch-profile-missing`. Other unsafe resolutions return stable codes including `runtime-unsupported`, `core-fingerprint-unknown`, `dependency-missing`, `manifest-checksum-unavailable`, and `content-set-mismatch`. Pass nested `client` and `runtimes` values exactly as the native client reports them. Logical aliases such as `ym2413.zip` exist only in strict resolved manifests and never rename the stored source asset. Returned file URLs remain bearer-authenticated and must be transferred outside MCP.
