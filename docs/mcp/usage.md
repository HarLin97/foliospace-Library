...
- `foliospace.list_games`: list paginated client-visible ROM assets with `limit`, `offset`, `q`, `platform`, `romSetName`, `format`, and `sort`. Use `platform: "cps1"`, `"cps2"`, or `"cps3"` for canonical Capcom CPS families, `platform: "mame"` for audited MAME sets, `platform: "model2"` for Sega Model 2, `platform: "n64"` for Nintendo 64, `platform: "psp"` for PSP, `platform: "ngc"` for Nintendo GameCube, `platform: "ps2"` for PlayStation 2, `platform: "pc98"` for NEC PC-98, and `platform: "dos"` for IBM-compatible DOS archives. Dependency packages are hidden. Games with `catalogRole: "needs-curation"` remain visible but may return `409 runtime-profile-not-available` from launch resolution until their runtime profile is audited. Validated metadata includes `fileName`, `format`, hashes, and an opaque authenticated `downloadUrl`.
