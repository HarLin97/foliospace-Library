...
- `foliospace.list_games`: list paginated client-safe ROM assets with `limit`, `offset`, `q`, `platform`, `romSetName`, `format`, and `sort`. Use `platform: "cps1"`, `"cps2"`, or `"cps3"` for canonical Capcom CPS families, `platform: "mame"` for audited MAME sets, `platform: "model2"` for Sega Model 2, `platform: "n64"` for Nintendo 64, `platform: "psp"` for PSP, `platform: "ngc"` for Nintendo GameCube, `platform: "ps2"` for PlayStation 2, `platform: "pc98"` for NEC PC-98, and `platform: "dos"` for IBM-compatible DOS archives. The tool returns only launch-ready games; dependency packages and `needs-curation` records remain indexed for backend audit but are hidden from MCP pages and searches. Validated metadata includes `fileName`, `format`, hashes, and an opaque authenticated `downloadUrl`.
