...
- `foliospace.open_game_manifest`: open a ROM client manifest by `gameId`. Multi-file Dreamcast GDI, Saturn CUE, and PC-FX CUE/M3U manifests include `entryFile` plus an ordered `files[]` list containing every descriptor and required track; transfer the returned HTTP URLs outside MCP and preserve each returned filename.
