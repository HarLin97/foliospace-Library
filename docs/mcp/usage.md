...
```text
https://foliospace.app/releases/foliospace-mcp_0.976_darwin_arm64.tar.gz
https://foliospace.app/releases/foliospace-mcp_0.976_darwin_amd64.tar.gz
https://foliospace.app/releases/foliospace-mcp_0.976_linux_arm64.tar.gz
https://foliospace.app/releases/foliospace-mcp_0.976_linux_amd64.tar.gz
...
- `foliospace.list_games`: list paginated client-safe ROM assets with `limit`, `offset`, `q`, `platform`, `romSetName`, `format`, and `sort`. Use `platform: "n64"` for canonical Nintendo 64 records and `platform: "pc98"` for canonical NEC PC-98 records. Validated raw-media metadata includes `fileName`, `format`, hashes, and an opaque authenticated `downloadUrl`.
- `foliospace.open_game_manifest`: open a ROM client manifest by `gameId`. Nintendo 64 and PC-98 manifests expose one validated raw ROM or disk-image entry even when the server stores it in a single-media ZIP. Multi-file Dreamcast GDI, Saturn CUE, and PC-FX CUE/M3U manifests include `entryFile` plus an ordered `files[]` list containing every descriptor and required track; transfer the returned HTTP URLs outside MCP and preserve each returned filename.
